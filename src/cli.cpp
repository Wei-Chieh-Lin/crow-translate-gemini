...
void Cli::process(const QCoreApplication &app)
{
    ...
    const QCommandLineOption engine({"e", "engine"}, tr("Specify the translator engine ('google', 'yandex', 'bing', 'libretranslate', 'lingva' or 'gemini'), Google is used by default."), QStringLiteral("engine"), QStringLiteral("google"));
    ...
    } else if (parser.value(engine) == QLatin1String("gemini")) {
        m_engine = QOnlineTranslator::Gemini;
        m_translator->setEngineApiKey(QOnlineTranslator::Engine::Gemini, AppSettings().engineApiKey(QOnlineTranslator::Engine::Gemini));
        m_translator->setGeminiModel(AppSettings().geminiModel());
    } else {
