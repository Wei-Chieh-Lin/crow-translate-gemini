...
void QOnlineTranslator::translate(const QString &text, Engine engine, Language translationLang, Language sourceLang, Language uiLang)
{
    ...
    case Gemini:
        if (m_geminiApiKey.isEmpty()) {
            resetData(ParametersError, tr("%1 API key can't be empty.").arg(QMetaEnum::fromType<Engine>().valueToKey(engine)));
            emit finished();
            return;
        }
        if (m_geminiModel.isEmpty())
            m_geminiModel = QStringLiteral("gemini-2.0-flash");
...
void QOnlineTranslator::setGeminiModel(const QString &model)
{
    m_geminiModel = model;
}
...
bool QOnlineTranslator::isSupportTranslation(Engine engine, Language lang)
{
    ...
    case LibreTranslate:
        ...
        break;
    case Gemini:
        // Gemini (LLM) can handle virtually any named language; reuse Google's exclusion list for safety.
        switch (lang) {
        case NoLanguage:
        case Bashkir:
        case Cantonese:
...
void QOnlineTranslator::requestGeminiTranslate()
{
    ...
    const QString sourceCode = m_sourceLang == Auto
        ? QStringLiteral("its auto-detected language")
        : languageApiCode(Gemini, m_sourceLang);
    const QString targetCode = languageApiCode(Gemini, m_translationLang);

    QString prompt;
    if (m_onlyDetectLanguage) {
        prompt = QStringLiteral(
                     "Detect the language of the following text. "
                     "Respond with ONLY the ISO 639-1 two-letter code (e.g. \"en\", \"zh\", \"ja\"), "
        ...
        prompt = QStringLiteral(
                     ...
                     "Do NOT include explanations, alternatives, quotes, or prefixes like \"Translation:\". "
                     "Preserve line breaks and formatting.\n\nText:\n%3")
                     .arg(sourceCode, targetCode, sourceText);
    }
    ...
    request.setRawHeader("x-goog-api-key", m_geminiApiKey);
...
void QOnlineTranslator::parseGeminiTranslate()
{
    ...
    if (m_currentReply->error() != QNetworkReply::NoError) {
        const QJsonDocument errorJson = QJsonDocument::fromJson(m_currentReply->readAll());
        const QString apiMessage = errorJson.object().value(QStringLiteral("error")).toObject().value(QStringLiteral("message")).toString();
        resetData(NetworkError, apiMessage.isEmpty() ? m_currentReply->errorString() : apiMessage);
        return;
    }

    const QJsonDocument jsonResponse = QJsonDocument::fromJson(m_currentReply->readAll());
    const QJsonArray candidates = jsonResponse.object().value(QStringLiteral("candidates")).toArray();
    if (candidates.isEmpty()) {
        resetData(ParsingError, tr("Gemini returned no candidates"));
        return;
    }

    const QJsonArray parts = candidates.first()
                                 .toObject()
    ...
    if (m_onlyDetectLanguage) {
        QString langCode = text.simplified();
        const int sep = langCode.indexOf(QLatin1Char(' '));
        if (sep >= 0)
            langCode = langCode.left(sep);
        langCode = langCode.toLower();
        const Language detected = language(Gemini, langCode);
        m_sourceLang = detected == NoLanguage ? English : detected;
        return;
    }
...
void QOnlineTranslator::buildGeminiStateMachine()
{
    ...
    // Setup translation state (Gemini handles large inputs natively)
    buildSplitNetworkRequest(translationState, &QOnlineTranslator::requestGeminiTranslate, &QOnlineTranslator::parseGeminiTranslate, m_source, s_geminiTranslateLimit);
}
...
void QOnlineTranslator::buildGeminiDetectStateMachine()
{
    ...
    // Setup lang detection state
    const QString text = m_source.left(getSplitIndex(m_source, s_geminiTranslateLimit));
    buildNetworkRequestState(detectState, &QOnlineTranslator::requestGeminiTranslate, &QOnlineTranslator::parseGeminiTranslate, text);
}
...
bool QOnlineTranslator::isSupportTranslit(Engine engine, Language lang)
{
    ...
    case Gemini: // Gemini returns plain translation only
        return false;
    }
